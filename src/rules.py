...
def process_attendance(date_str):
    ...
    with get_connection() as conn:
        ...
        for agent in agents:
            ...
            if sched_end and last_logout:
                try:
                    l_out = datetime.strptime(last_logout, "%H:%M").time()
                    diff_end = (datetime.combine(datetime.min, sched_end) - datetime.combine(datetime.min, l_out)).total_seconds() / 60
                    if diff_end > 15:
                        adherence_detail += f"Salida temprana: {int(diff_end)} min. "
                except: pass
